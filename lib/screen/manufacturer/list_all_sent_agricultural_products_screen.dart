...
class ListAllSentAgriculturalProductsScreen extends StatefulWidget {
  ...
  @override
  State<ListAllSentAgriculturalProductsScreen> createState() => _ListAllSentAgriculturalProductsScreenState();
}
...
class _ListAllSentAgriculturalProductsScreenState extends State<ListAllSentAgriculturalProductsScreen> {
 ...
 var dateFormat = DateFormat('dd-MM-yyyy');
   ...
   void fetchData () async {
    ...
    raw_material_shippings = await rawMaterialShippingController.getListAllSentAgriByUsername(username);
    setState(() {
      isLoaded = true;
    });
    ...
    return  GestureDetector(
      ...
      child: SafeArea(
        child: Scaffold(
          ...
          raw_material_shippings?.length == 0?
          Container(
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    title: Column(
                      ...
                      children: [
                        Text(
                          "ส่ง : "+"${raw_material_shippings?[index].planting?.plantName}",
                          ...
                          Text(
                          "รหัสการส่งผลผลิต :"+"${raw_material_shippings?[index].planting?.plantingId}",
                          style: const TextStyle(
                            fontFamily: 'Itim',
                            fontSize: 22
                          ),
                          ...
                          Text(
                          "จาก :"+"${raw_material_shippings?[index].planting?.farmer?.farmerName}"+" "+
                          "${raw_material_shippings?[index].planting?.farmer?.farmerLastname}",
                          style: const TextStyle(
                            fontFamily: 'Itim',
                            fontSize: 22
                          ),
                        ...
                        Text(
                          "วันที่ส่ง : "+"${dateFormat.format(raw_material_shippings?[index].rawMatShpDate?? DateTime.now())}"+
                          " จำนวน : "+"${raw_material_shippings?[index].rawMatShpQty}"+" "+"${raw_material_shippings?[index].rawMatShpQtyUnit}",
                          style: const TextStyle(
                            fontFamily: 'Itim',
                            fontSize: 22
                          ),
